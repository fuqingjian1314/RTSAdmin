				  	...
				  	<table class="table table-bordered table-hover"  id="table_url_fqj">
					   <thead>
					      <tr>
					         ...
					         <th field="method">访问方法</th>
					         <th field="createby">访问者</th>
					         <th field="formatCreateDate">访问时间</th>
...
function loadPager(pageNum){
	...
	$.post(url,{offSet:offSet,pageSize:15,logType:0},function(rows){
...
function loadPaging(pageNum){
	...
	$.post(url,date,function(data){
		...
		$("#table_url_fqj").table(rows);
		//初始化分页插件
		$("#tablePaging").tablePaging(1,15,count,"loadPager");
		//隐藏多选列
		$('table tr').find('td:eq(0)').hide();
	});
}
