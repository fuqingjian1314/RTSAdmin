...
(function( $ ) {
		...
		*/
        diyUpload:function( opt, serverCallBack ) {
			...
			webUploader.on('fileQueued', function( file ) {
				//获取队列中的文件
				//TODO 这里因为队列不能清空，取消后再上传会有问题，现在先暂时放到后期处理
				var qfileArr = webUploader.getFiles('queued');
				var fileArr = $("input[name="+opt.inputName+"]");
				var length = opt.fileNumLimit - fileArr.length - qfileArr.length;
				if(length < 0){
			...
			webUploader.on('uploadSuccess',function( file, response ){
				...
				var inputStr = '<input type="hidden" value="'+data.imageName+'" name="'+opt.inputName+'">';
				$fileBox.append(inputStr);
				var $diyBar = $fileBox.find('.diyBar');	
				$fileBox.removeClass('diyUploadHover');
				$diyBar.fadeOut( 1000 ,function(){
					$fileBox.children('.diySuccess').show();
					$fileBox.children('.diyDelete').show();
					var $diyDelete = $fileBox.children('.diyDelete').on('click',function(){
						layer.msg('确认删除图片吗?',{time:0,btn:['确定', '取消'],yes: function(index){
							layer.close(index);
							$diyBar.show();
							showDiyProgress( 60, $diyBar , '文件删除中!' );
							deleteUploadFile(data.imageName,$fileBox,opt.deleteUrl,file.id,true,webUploader,opt);
						  }
						});
					});
				});
			...
			webUploader.on('uploadError',function( file, reason ){
				...
				var err = '文件:'+file.name+'由于插件上传插件出错,上传失败,请重新上传!';
				if ( errorCallBack ) {
					errorCallBack( err );
				}
			});
	...
	function createBox( $fileInput, file, webUploader ) {
		...
		if ( $parentFileBox.length <= 0 ) {
			...
			var div = '<div class="parentFileBox"> \
						<ul class="fileBoxUl" style="margin-left: -44px;"></ul>\
					</div>';
			$fileInput.after( div );
		...
		if ( $parentFileBox.find('.diyButton').length <= 0 ) {
			...
			var uploadStart = function (){
				webUploader.upload();
				$parentFileBox.find('.diyCancelAll').hide();
				/*$(".diyCancelAll").hide();*/
			...
			$cancelButton.bind('click',function(){
				layer.msg('确定取消所有的图片上传吗?',{time:0,btn:['确定', '取消'],yes: function(index){
					layer.close(index);
		...
		//添加子容器;  <div class="diySuccess icon-ok iop" title="上传成功"></div> \
		var li = '<li id="fileBox_'+file_id+'" class="diyUploadHover"> \
					<div class="viewThumb"></div> \
					<div class="diyCancel icon-remove iop" title="取消上传"></div> \
					<div class="diyDelete icon-trash iop"  title="删除文件"></div> \
		...
		// debugger
		//$parentFileBox.width( $width );  由于和 inline-block 冲突  强制使用  165 px 
		$parentFileBox.width(165);
		...
		var $diyCancel = $fileBox.children('.diyCancel').on('click',function(){
			...
			layer.msg('确定取消图片上传吗?',{time:0,btn:['确定', '取消'],yes: function(index){
				layer.close(index);
				var cobj=$diyCancel;
				
				removeLi(obj,file_id, webUploader);	 
			  }
			});
	...
	//删除文件方法
	function deleteUploadFile(fileName,obj,deleteUrl,fileId,isAdd,webUploader,opt){
		var queryData = {};
		queryData.fileName = fileName;
		$.post(deleteUrl,queryData,function(data){
			if(data.status){
				if(isAdd){
					removeLi(obj,fileId,webUploader);
				} else {
					removeLi(obj,fileId,webUploader);
					obj.remove();
				}
